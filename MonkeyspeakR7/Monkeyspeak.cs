...
namespace Monkeyspeak
{
    ...
    public sealed class MonkeyspeakEngine
    {
        ...
        /// <param name="chunk">String that contains the Monkeyspeak script source.</param>
        /// <returns></returns>
        public async Task<Page> LoadFromStringAsync(Page existingPage, string chunk)
        {
            return await Task.Run(() => LoadFromString(existingPage, chunk));
        }
        ...
        /// <param name="chunk">String that contains the Monkeyspeak script source.</param>
        /// <returns></returns>
        public Page LoadFromString(Page existingPage, string chunk)
        {
            try
            {
                var stream = new MemoryStream();
        ...
        /// <param name="stream">Stream that contains the Monkeyspeak script. Closes the stream.</param>
        /// <returns><see cref="Monkeyspeak.Page"/></returns>
        public async Task<Page> LoadFromStreamAsync(Page existingPage, Stream stream)
        {
            return await Task.Run(() => LoadFromStream(existingPage, stream));
        }
        ...
        /// <param name="existingPage">Reference to an existing Page</param>
        /// <param name="stream">Stream that contains the Monkeyspeak script. Closes the stream.</param>
        public Page LoadFromStream(Page existingPage, Stream stream)
        {
            try
            {
                using (var reader = new SStreamReader(stream, true))
